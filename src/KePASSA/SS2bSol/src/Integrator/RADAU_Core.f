      ...
      INTEGER, SAVE :: ConUnit
      REAL,    SAVE :: InitialStepsize

      CONTAINS



      SUBROUTINE RADAU_Integrator
     &                 (N,FCN,X,Y,XEND,H,
     &                  RTOL,ATOL,ITOL,
...
C -------- NUMBER MAXIMAL AND MINIMAL OF STAGES  NS
      ...
      ELSE
         ...
         IF (NSUS.LE.0.OR.NS.GE.8.OR.NS.EQ.2.OR.NS.EQ.4.OR.NS.EQ.6) THEN
            WRITE(ConUnit,*)' WRONG INPUT IWORK(13)=',IWORK(13)
            ARRET=.TRUE.
         END IF
      END IF
C -------- NMAX , THE MAXIMAL NUMBER OF STEPS -----
      ...
      ELSE
         NMAX=IWORK(2)
         IF (NMAX.LE.0) THEN
            WRITE(ConUnit,*)' WRONG INPUT IWORK(2)=',IWORK(2)
            ARRET=.TRUE.
         END IF
      END IF
C -------- NIT    MAXIMAL NUMBER OF NEWTON ITERATIONS
      ...
      ELSE
         ...
         IF (NIT.LE.0.OR.NIT.GT.50) THEN
            WRITE(ConUnit,*)' CURIOUS INPUT IWORK(3)=',IWORK(3)
            ARRET=.TRUE.
         END IF
      END IF
...
C -------- PARAMETER FOR DIFFERENTIAL-ALGEBRAIC COMPONENTS
      ...
      IF (NIND1+NIND2+NIND3.NE.N) THEN
       WRITE(ConUnit,*)' CURIOUS INPUT FOR IWORK(5,6,7)=',NIND1,NIND2,NIND3
       ARRET=.TRUE.
      END IF
...
C -------- PARAMETER FOR SECOND ORDER EQUATIONS
      ...
      IF (M1.LT.0.OR.M2.LT.0.OR.M1+M2.GT.N) THEN
       WRITE(ConUnit,*)' CURIOUS INPUT FOR IWORK(9,10)=',M1,M2
       ARRET=.TRUE.
      END IF
C -------- UROUND   SMALLEST NUMBER SATISFYING   1. + UROUND > 1. 
      ...
      ELSE
         UROUND=WORK(1)
         !IF (UROUND.LE.1.e-19.OR.UROUND.GE.1.) THEN
         IF (UROUND .LE. 1.e-3*epsilon(0.) .OR. UROUND .GE. 1.) THEN
            WRITE(ConUnit,*)' COEFFICIENTS HAVE 20 DIGITS, UROUND=',
     &                      WORK(1)
            ARRET=.TRUE.
         END IF
      END IF
C --------- CHECK IF TOLERANCES ARE O.K.
      IF (ITOL.EQ.0) THEN
          IF (ATOL(1).LE.0.0.OR.RTOL(1).LE.10.*UROUND) THEN
              WRITE (ConUnit,*) ' TOLERANCES ARE TOO SMALL'
              ARRET=.TRUE.
          END IF
      ELSE
          DO I=1,N
          IF (ATOL(I).LE.0.0.OR.RTOL(I).LE.10.*UROUND) THEN
              WRITE (ConUnit,*) ' TOLERANCES(',I,') ARE TOO SMALL'
              ARRET=.TRUE.
          END IF
          END DO
      END IF
C --------- SAFE     SAFETY FACTOR IN STEP SIZE PREDICTION
      ...
      ELSE
         SAFE=WORK(2)
         IF (SAFE.LE.0.001.OR.SAFE.GE.1.) THEN
            WRITE(ConUnit,*)' CURIOUS INPUT FOR WORK(2)=',WORK(2)
            ARRET=.TRUE.
         END IF
      END IF
C ------ THET     DECIDES WHETHER THE JACOBIAN SHOULD BE RECOMPUTED;
      ...
      ELSE
         THET=WORK(3)
         IF (THET.GE.1.) THEN
            WRITE(ConUnit,*)' CURIOUS INPUT FOR WORK(3)=',WORK(3)
            ARRET=.TRUE.
         END IF
      END IF
C --- QUOT1 AND QUOT2: IF QUOT1 < HNEW/HOLD < QUOT2, STEP SIZE = CONST.
      ...
      ELSE
         QUOT2=WORK(6)
      END IF
      IF (QUOT1.GT.1.0.OR.QUOT2.LT.1.0) THEN
         WRITE(ConUnit,*)' CURIOUS INPUT FOR WORK(5,6)=',QUOT1,QUOT2
         ARRET=.TRUE.
      END IF
C -------- MAXIMAL STEP SIZE
      IF (WORK(7).EQ.0.) THEN
...
C -------  FACL,FACR     PARAMETERS FOR STEP SIZE SELECTION
      ...
      ELSE
         FACR=1./WORK(9)
      END IF
      IF (FACL.LT.1.0.OR.FACR.GT.1.0) THEN
            WRITE(ConUnit,*)' CURIOUS INPUT WORK(8,9)=',WORK(8),WORK(9)
            ARRET=.TRUE.
         END IF
C -------- PARAMETERS FOR ORDER SELECTION STRATEGY
      IF (WORK(10).EQ.0.) THEN
         VITU=0.002
      ELSE
         VITU=WORK(10)
      END IF 
      IF (WORK(11).EQ.0.) THEN
         VITD=0.8
      ELSE
         VITD=WORK(11)
      END IF 
      IF (WORK(12).EQ.0.) THEN
         HHOU=1.2
      ELSE
         HHOU=WORK(12)
      END IF 
      IF (WORK(13).EQ.0.) THEN
         HHOD=0.8
      ELSE
         HHOD=WORK(13)
      END IF 
...
C ------ BANDWITH OF "MAS" NOT SMALLER THAN BANDWITH OF "JAC"
          IF (MLMAS.GT.MLJAC.OR.MUMAS.GT.MUJAC) THEN
             WRITE (ConUnit,*) 'BANDWITH OF "MAS" NOT SMALLER THAN BANDWITH OF
     & "JAC"'
            ARRET=.TRUE.
          END IF
      ELSE
...
C ------ HESSENBERG OPTION ONLY FOR EXPLICIT EQU. WITH FULL JACOBIAN
      IF ((IMPLCT.OR.JBAND).AND.IJOB.EQ.7) THEN
         WRITE(ConUnit,*)' HESSENBERG OPTION ONLY FOR EXPLICIT EQUATIONS WITH 
     &FULL JACOBIAN'
         ARRET=.TRUE.
      END IF
...
C ------ TOTAL STORAGE REQUIREMENT -----------
      ...
      IF(ISTORE.GT.LWORK)THEN
         WRITE(ConUnit,*)' INSUFFICIENT STORAGE FOR WORK, MIN. LWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
C --------- TOTAL REQUIREMENT ---------------
      ...
      IF (ISTORE.GT.LIWORK) THEN
         WRITE(ConUnit,*)' INSUFF. STORAGE FOR IWORK, MIN. LIWORK=',ISTORE
         ARRET=.TRUE.
      END IF
...
C --- THIS PARAMETER HAS TO BE CHANGED IF NUMBER OF STAGES IS >=7
      ...
      COMMON/WEIGHT/XSOL,HSOL,C(0:NSDIM),NN,NSCON
...
C ---------- CONSTANTS ---------
      ...
      POSNEG=SIGN(1.,XEND-X)
      HMAXN=MIN(ABS(HMAX),ABS(XEND-X)) 
      IF (ABS(H).LE.10.*UROUND) H=1.e-6
      H=MIN(ABS(H),HMAXN)
      H=SIGN(H,POSNEG)
      HOLD=H
      REJECT=.FALSE.
      FIRST=.TRUE.
      LAST=.FALSE.
      IF ((X+H*1.0001-XEND)*POSNEG.GE.0.) THEN
         H=XEND-X
         LAST=.TRUE.
      END IF
      HOPT=H
     ...
     &            RPAR,IPAR,IRTRN,
     &            [0, 0, 0, 0],
     &            [NFCN,NJAC,NACCPT,NREJCT,NDEC,NSOL] )
...
C --- JACOBIAN IS BANDED
            ...
            DO MM=1,M1/M2+1
               DO K=1,MD
                  J=K+(MM-1)*M2
 12               FF(J)=Y(J)
                  FF(J+N)=SQRT(UROUND*MAX(1.e-5,ABS(Y(J))))
                  Y(J)=Y(J)+FF(J+N)
                  J=J+MD
...
C --- JACOBIAN IS FULL
            DO I=1,N
               YSAFE=Y(I)
               DELT=SQRT(UROUND*MAX(1.e-5,ABS(YSAFE)))
               Y(I)=YSAFE+DELT
               CALL FCN(N,X,Y,CONT,RPAR,IPAR)
               DO J=M1+1,N
...
C --- COMPUTE THE MATRICES E1 AND E2 AND THEIR DECOMPOSITIONS
     ...
     &            M1,M2,NM1,FAC1,E1,LDE1,IP1,IER,IJOB,CALHES,IPHES)
      IF (IER.NE.0 .AND. (.NOT.FixedStepsize)) GOTO 78
      DO K=1,(NS-1)/2
         ALPHN(K)=ALPH(K)/H
         BETAN(K)=BETA(K)/H
         IAD=(K-1)*2*NM1+1
  ...
  30  CONTINUE
      ...
      IF (0.1*ABS(H).LE.ABS(X)*UROUND) GOTO 177
          IF (INDEX2) THEN
...
C *** *** *** *** *** *** ***
      ...
      NIT=NIT1
      EXPMI=1./EXPMNS
      FNEWT=MAX(10*UROUND/RTOL1,MIN(0.03,RTOL1**(EXPMI-1.)))
      FACCON=MAX(FACCON,UROUND)**0.8
      THETA=ABS(THET)
  40  CONTINUE
      IF (NEWT.GE.NIT .AND. (.NOT.FixedStepsize)) GOTO 78
...
C ---     BAD CONVERGENCE OR NUMBER OF ITERATIONS TO LARGE
      ...
      IF (FACCON*DYNO.GT.FNEWT) Then
          If (.NOT.FixedStepsize .OR. 
     &        (FixedStepsize .AND. NEWT.LE.NIT)
     &       ) GOTO 40
      End If
C --- ERROR ESTIMATION  
...
C       --- COMPUTE FINITE DIFFERENCES FOR DENSE OUTPUT
      IF (ERR.LT.1. .OR. FixedStepsize) THEN
         DO I=1,N
            Y(I)=Y(I)+ZZ(I+N2)  
            Z2I=ZZ(I+N)
            Z1I=ZZ(I)
...
C *** *** *** *** *** *** ***
      ...
      NIT=NIT1-3
      EXPMI=1.0/EXPMNS
      FNEWT=MAX(10*UROUND/RTOL1,0.03)
      FACCON=MAX(FACCON,UROUND)**0.8
      THETA=ABS(THET)
 440  CONTINUE
      IF (NEWT.GE.NIT .AND. (.NOT.FixedStepsize)) GOTO 78
...
C --- WE REQUIRE .2<=HNEW/H<=8.
      ...
      HNEW=H/QUOT
      IF (FixedStepsize) HNEW = InitialStepsize  ! If requested, force fixed-stepsize
C *** *** *** *** *** *** ***
...
C       --- UPDATE SCALING
     ...
     &               [NFCN,NJAC,NACCPT,NREJCT,NDEC,NSOL] )
         ...
         IF (FixedStepsize) HNEW = InitialStepsize  ! If requested, force fixed-stepsize
         REJECT=.FALSE.
         !IF ((X+HNEW/QUOT1-XEND)*POSNEG.GE.0.) THEN
         IF ((X+HNEW/QUOT1-XEND)*POSNEG.GE.-10*UROUND) THEN ! Stretch last stepsize if XEND can be reached within a distance of 10*UROUND
            H=XEND-X                                        ! This avoids the possibility of the very final stepsize becoming too small
            LAST=.TRUE.                                     ! and crashing the integration.
         ELSE
            QT=HNEW/H 
            HHFAC=H
...
C --- FAIL EXIT
 176  CONTINUE
      WRITE(ConUnit,979)X   
      WRITE(ConUnit,*) ' MATRIX IS REPEATEDLY SINGULAR, IER=',IER
      IDID=-4
      RETURN
 177  CONTINUE
      WRITE(ConUnit,979)X   
      WRITE(ConUnit,*) ' STEP SIZE T0O SMALL, H=',H
      IDID=-3
      RETURN
 178  CONTINUE
      WRITE(ConUnit,979)X   
      WRITE(ConUnit,*) ' MORE THAN NMAX =',NMAX,'STEPS ARE NEEDED' 
      IDID=-2
      RETURN
...
C
     ...
     *              TI771,TI772,TI773,TI774,TI775,TI776,TI777
C ---
      T311 =  0.9123239487089294279155e-01
      T312 = -0.1412552950209542084280e+00
      T313 = -0.3002919410514742449186e-01
      T321 =  0.2417179327071070189575e+00
      T322 =  0.2041293522937999319960e+00
      T323 =  0.3829421127572619377954e+00
      T331 =  0.9660481826150929361906e+00
      TI311 =  0.4325579890063155351024e+01
      TI312 =  0.3391992518158098695428e+00
      TI313 =  0.5417705399358748711865e+00
      TI321 = -0.4178718591551904727346e+01
      TI322 = -0.3276828207610623870825e+00
      TI323 =  0.4766235545005504519601e+00
      TI331 = -0.5028726349457868759512e+00
      TI332 =  0.2571926949855605429187e+01
      TI333 = -0.5960392048282249249688e+00
      IF (NSMAX.LE.3) RETURN
      T511 = -0.1251758622050104589014e-01
      T512 = -0.1024204781790882707009e-01
      T513 =  0.4767387729029572386318e-01
      T514 = -0.1147851525522951470794e-01
      T515 = -0.1401985889287541028108e-01
      T521 = -0.1491670151895382429004e-02
      T522 =  0.5017286451737105816299e-01
      T523 = -0.9433181918161143698066e-01
      T524 = -0.7668830749180162885157e-02
      T525 =  0.2470857842651852681253e-01
      T531 = -0.7298187638808714862266e-01
      T532 = -0.2305395340434179467214e+00
      T533 =  0.1027030453801258997922e+00
      T534 =  0.1939846399882895091122e-01
      T535 =  0.8180035370375117083639e-01
      T541 = -0.3800914400035681041264e+00
      T542 =  0.3778939022488612495439e+00
      T543 =  0.4667441303324943592896e+00
      T544 =  0.4076011712801990666217e+00
      T545 =  0.1996824278868025259365e+00
      T551 = -0.9219789736812104884883e+00
      TI511 = -0.3004156772154440162771e+02
      TI512 = -0.1386510785627141316518e+02
      TI513 = -0.3480002774795185561828e+01
      TI514 =  0.1032008797825263422771e+01
      TI515 = -0.8043030450739899174753e+00
      TI521 =  0.5344186437834911598895e+01
      TI522 =  0.4593615567759161004454e+01
      TI523 = -0.3036360323459424298646e+01
      TI524 =  0.1050660190231458863860e+01
      TI525 = -0.2727786118642962705386e+00
      TI531 =  0.3748059807439804860051e+01
      TI532 = -0.3984965736343884667252e+01
      TI533 = -0.1044415641608018792942e+01
      TI534 =  0.1184098568137948487231e+01
      TI535 = -0.4499177701567803688988e+00
      TI541 = -0.3304188021351900000806e+02
      TI542 = -0.1737695347906356701945e+02
      TI543 = -0.1721290632540055611515e+00
      TI544 = -0.9916977798254264258817e-01
      TI545 =  0.5312281158383066671849e+00
      TI551 = -0.8611443979875291977700e+01
      TI552 =  0.9699991409528808231336e+01
      TI553 =  0.1914728639696874284851e+01
      TI554 =  0.2418692006084940026427e+01
      TI555 = -0.1047463487935337418694e+01
      IF (NSMAX.LE.5) RETURN
      T711 = -0.2153754627310526422828e-02
      T712 =  0.2156755135132077338691e-01
      T713 =  0.8783567925144144407326e-02
      T714 = -0.4055161452331023898198e-02
      T715 =  0.4427232753268285479678e-02
      T716 = -0.1238646187952874056377e-02
      T717 = -0.2760617480543852499548e-02
      T721 =  0.1600025077880428526831e-02
      T722 = -0.3813164813441154669442e-01
      T723 = -0.2152556059400687552385e-01
      T724 =  0.8415568276559589237177e-02
      T725 = -0.4031949570224549492304e-02
      T726 = -0.6666635339396338181761e-04
      T727 =  0.3185474825166209848748e-02
      T731 = -0.4059107301947683091650e-02
      T732 =  0.5739650893938171539757e-01
      T733 =  0.5885052920842679105612e-01
      T734 = -0.8560431061603432060177e-02
      T735 = -0.6923212665023908924141e-02
      T736 = -0.2352180982943338340535e-02
      T737 =  0.4169077725297562691409e-03
      T741 = -0.1575048807937684420346e-01
      T742 = -0.3821469359696835048464e-01
      T743 = -0.1657368112729438512412e+00
      T744 = -0.3737124230238445741907e-01
      T745 =  0.8239007298507719404499e-02
      T746 =  0.3115071152346175252726e-02
      T747 =  0.2511660491343882192836e-01
      T751 = -0.1129776610242208076086e+00
      T752 = -0.2491742124652636863308e+00
      T753 =  0.2735633057986623212132e+00
      T754 =  0.5366761379181770094279e-02
      T755 =  0.1932111161012620144312e+00
      T756 =  0.1017177324817151468081e+00
      T757 =  0.9504502035604622821039e-01
      T761 = -0.4583810431839315010281e+00
      T762 =  0.5315846490836284292051e+00
      T763 =  0.4863228366175728940567e+00
      T764 =  0.5265742264584492629141e+00
      T765 =  0.2755343949896258141929e+00
      T766 =  0.5217519452747652852946e+00
      T767 =  0.1280719446355438944141e+00
      T771 = -0.8813915783538183763135e+00
      TI711 = -0.2581319263199822292761e+03
      TI712 = -0.1890737630813985089520e+03
      TI713 = -0.4908731481793013119445e+02
      TI714 = -0.4110647469661428418112e+01
      TI715 = -0.4053447889315563304175e+01
      TI716 =  0.3112755366607346076554e+01
      TI717 = -0.1646774913558444650169e+01
      TI721 = -0.3007390169451292131731e+01
      TI722 = -0.1101586607876577132911e+02
      TI723 =  0.1487799456131656281486e+01
      TI724 =  0.2130388159559282459432e+01
      TI725 = -0.1816141086817565624822e+01
      TI726 =  0.1134325587895161100083e+01
      TI727 = -0.4146990459433035319930e+00
      TI731 = -0.8441963188321084681757e+01
      TI732 = -0.6505252740575150028169e+00
      TI733 =  0.6940670730369876478804e+01
      TI734 = -0.3205047525597898431565e+01
      TI735 =  0.1071280943546478589783e+01
      TI736 = -0.3548507491216221879730e+00
      TI737 =  0.9198549132786554154409e-01
      TI741 =  0.7467833223502269977153e+02
      TI742 =  0.8740858897990081640204e+02
      TI743 =  0.4024158737379997877014e+01
      TI744 = -0.3714806315158364186639e+01
      TI745 = -0.3430093985982317350741e+01
      TI746 =  0.2696604809765312378853e+01
      TI747 = -0.9386927436075461933568e+00
      TI751 =  0.5835652885190657724237e+02
      TI752 = -0.1006877395780018096325e+02
      TI753 = -0.3036638884256667120811e+02
      TI754 = -0.1020020865184865985027e+01
      TI755 = -0.1124175003784249621267e+00
      TI756 =  0.1890640831000377622800e+01
      TI757 = -0.9716486393831482282172e+00
      TI761 = -0.2991862480282520966786e+03
      TI762 = -0.2430407453687447911819e+03
      TI763 = -0.4877710407803786921219e+02
      TI764 = -0.2038671905741934405280e+01
      TI765 =  0.1673560239861084944268e+01
      TI766 = -0.1087374032057106164456e+01
      TI767 =  0.9019382492960993738427e+00
      TI771 = -0.9307650289743530591157e+02
      TI772 =  0.2388163105628114427703e+02
      TI773 =  0.3927888073081384382710e+02
      TI774 =  0.1438891568549108006988e+02
      TI775 = -0.3510438399399361221087e+01
      TI776 =  0.4863284885566180701215e+01
      TI777 = -0.2246482729591239916400e+01
      RETURN
      END
C
...
C
 ...
 11   CONTINUE
  ...
  5   CONTINUE
      ...
      DD(1)= -0.2778093394406463730479e+02
      DD(2)=  0.3641478498049213152712e+01
      DD(3)= -0.1252547721169118720491e+01
      DD(4)=  0.5920031671845428725662e+00
      DD(5)= -0.2000000000000000000000e+00
      U1=  0.6286704751729276645173e+01
      ALPH(1)=  0.3655694325463572258243e+01
      BETA(1)=  0.6543736899360077294021e+01
      ALPH(2)=  0.5700953298671789419170e+01
      BETA(2)=  0.3210265600308549888425e+01
      RETURN
  7   CONTINUE
      ...
      DD(1)= -0.5437443689412861451458e+02
      DD(2)=  0.7000024004259186512041e+01
      DD(3)= -0.2355661091987557192256e+01
      DD(4)=  0.1132289066106134386384e+01
      DD(5)= -0.6468913267673587118673e+00
      DD(6)=  0.3875333853753523774248e+00
      DD(7)= -0.1428571428571428571429e+00
      U1=  0.8936832788405216337302e+01
      ALPH(1)=  0.4378693561506806002523e+01
      BETA(1)=  0.1016969328379501162732e+02
      ALPH(2)=  0.7141055219187640105775e+01
      BETA(2)=  0.6623045922639275970621e+01
      ALPH(3)=  0.8511834825102945723051e+01
      BETA(3)=  0.3281013624325058830036e+01
      RETURN
      END
C
...
C ----------------------------------------------------------
      ...
      IMPLICIT REAL (A-H,O-Z)
      DIMENSION CONT(LRC)
      COMMON/WEIGHT/XSOL,HSOL,C(0:NSDIM),NN,NS
      S=(X-XSOL)/HSOL+1.
      CONTRA=CONT(I+NS*NN)
      DO K=NS-1,0,-1
